...
assert(MIDSECTION.OUTER.WIDTH.isEqual(new Equation(
  MIDSECTION.PANELS.FRONT.width,
  MIDSECTION.PANELS.FRONT.cutoutWidth.multiply(-1),
  MIDSECTION.PANELS.CUTOUT_BACK.width,
  topw.multiply(-2)
)), 'width at panels')
...
assert(MIDSECTION.PANELS.CUTOUT_TOP.width.isEqual(
  MIDSECTION.PANELS.CUTOUT_BACK.width
),'cutout top width is same as back width');

// CUTOUT HEIGHT
assert(MIDSECTION.PANELS.CUTOUT_BACK.height.isEqual(
  MIDSECTION.PANELS.CUTOUT_SIDE.height
), 'cutout side and back have same height');
